#include "PauseMenu.h"
#include <iostream>
PauseMenu::PauseMenu(ID3D11Device* device)
{
	check = 0;
	renderMenu = false;
	currentSelect = 0;
...
void PauseMenu::Update(bool up, bool down, bool enter)
{
	// 0 = start & quit = 1
...
void PauseMenu::Render(ID3D11DeviceContext* deviceContext)
{
...
void PauseMenu::CheckIfToPause(bool gamepadStart)
{
	if (gamepadStart == true && pauseCheck == 0)
	{
		pauseCheck = 1;
		renderMenu = true;
	}
...
Geometry* PauseMenu::GetButtonGeometry()
{
	return buttonGeometry;
}

void PauseMenu::UpdateConstantBuffer(ID3D11DeviceContext* deviceContext, ButtonScale* buffer)
{
